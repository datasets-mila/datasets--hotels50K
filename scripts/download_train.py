# Minor modifications compared to the original script provided by Hotel-50k
from __future__ import print_function
import csv, multiprocessing, cv2, io, os, tarfile
import numpy as np
import urllib
...
def main():
    with tarfile.open('Hotels-50K/input/dataset.tar.gz', 'r:gz') as tf:
        hotel_stream = io.StringIO(tf.extractfile('dataset/hotel_info.csv').read().decode())
        hotel_reader = csv.reader(hotel_stream)
        train_stream = io.StringIO(tf.extractfile('dataset/train_set.csv').read().decode())
        train_reader = csv.reader(train_stream)
    ...
    # There is no header in dataset/train_set.csv
